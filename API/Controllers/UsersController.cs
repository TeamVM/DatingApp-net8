    ...
    IMapper mapper, IPhotoService photoService) : BaseApiController
{
    ...
    public async Task<ActionResult<PhotoDto>> AddPhoto(IFormFile file)
    {
        ...
        if (result.Error != null) return BadRequest(result.Error); //for error cloud name api key secret
        ...
        if (await userRepository.SaveAllAsync())
            return CreatedAtAction(nameof(GetUser),new {username = user.UserName}, mapper.Map<PhotoDto
                >(photo));
